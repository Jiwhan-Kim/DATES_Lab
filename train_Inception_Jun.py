...
if __name__ == "__main__":
    ...
    model   = M.Inception_Jun().to(device)
    trainer = T.SGDMC_Trainer_Inception_Jun(0.1, model, device)
    ...
    """if path.exists("./model_params_ResNet.pth"):
        model.load_state_dict(torch.load("./model_params_Inception_Jun.pth"))"""
    ...
    for i in range(epoch):
        ...
        if i==0:
          ...
          torch.save(model.state_dict(), 'model_params_Inception_Jun.pth')
        ...
        if no_improvement_count >= patience and trainer.lr > 0.001:
            ...
            model.load_state_dict(torch.load("./model_params_Inception_Jun.pth")) # take best one
            print("LR decreased")
